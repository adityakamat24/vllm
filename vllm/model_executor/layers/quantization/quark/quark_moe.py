...
class QuarkOCP_MX_MoEMethod(QuarkMoEMethod):
    ...
    def _quantize_activations(self, hidden_states: torch.Tensor) -> torch.Tensor:
        """
        Quantize activations using MXFP4/MXFP6 quantize-dequantize for emulation.

        Args:
            hidden_states: Input activations

        Returns:
            Quantized and dequantized activations (emulation)
        """
        if self.input_dtype == "mxfp4":
            return quant_dequant_mxfp4(hidden_states)
        elif self.input_dtype == "mxfp6_e3m2":
            return quant_dequant_mxfp6(hidden_states, quant_dtype="fp6_e3m2")
    ...
    def get_fused_moe_quant_config(
        ...
        if self.emulate:
            # For emulation, weights and activations are pre-processed in apply()
            # Return None to signal fused_experts that no quantization is needed
            return None
        else:
            # For native AITER path, return the OCP MX config
    ...
    def apply(
        ...
        if not self.emulate:
            ...
            # Dequantize weights for MXFP4/MXFP6 emulation
            ...
            # Quantize activations (QDQ emulation)
            quantized_x = self._quantize_activations(x)

            # Call fused_experts with dequantized weights and quantized activations
            out = fused_experts(
                quantized_x,
                dequant_w1,
                dequant_w2,
